...
import CraftingTaskCard from "@pages/auth/skill/components/task/CraftingTaskCard.tsx";
import useSmartRefWatcher from "@hooks/smartref/useSmartRefWatcher.ts";
import { GameData } from "@idleclient/game/data/GameData.ts";
import BasicTaskHeader from "@pages/auth/skill/components/BasicTaskHeader.tsx";
import GatheringTaskCard from "@pages/auth/skill/components/task/GatheringTaskCard.tsx";
			...
			>
				<div className="max-w-[calc(100% - 122px)] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
					...
					{ categories?.[currentCategory]?.tasks?.filter(v => !v.disabled).map((task, i) => (
						...
						<>
							{ task.isHarvestingTask() && task.costs.length === 0 ?
